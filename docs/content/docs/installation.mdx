---
title: Installing the React Native Barcode Scanner
description: How to install react-native-scanner-pro, the React Native QR code and barcode scanner library, and set up camera permissions on iOS and Android.
icon: PackageCheck
---
...
Face detection is bundled in the library (ML Kit on Android, Vision on iOS). No extra install step — set `detectionType="face"` and `cameraPosition="front"` on `<Scanner>`. See [Face Detection](/docs/features/face-detection) for details.
