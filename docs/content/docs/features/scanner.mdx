---
title: Scanner Component for QR Codes & Barcodes
description: The main React Native component for scanning QR codes and barcodes on both iOS and Android.
icon: Camera
---
...
| `cameraPosition` | `'back' \| 'front'` | `'back'` | Camera facing (front for selfies / face mode) |
| `faceDetection` | `FaceDetectionConfig` | — | Face overlay styling (face mode) |
| `onFacesDetected` | `(event) => void` | — | Callback with detected faces (face mode) |

---

## Face detection

Set `detectionType="face"` to switch from barcode scanning to on-device face detection. Use the front camera and listen for `onFacesDetected`:
...
See the full [Face Detection guide](/docs/features/face-detection) for overlay options, callback payload, and platform notes.
